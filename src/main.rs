...
fn main() {
		...
		for b in -11..11 {
			...
			if (center - vec3(4.0, 0.2, 0.0)).magnitude() > 0.9 {
				...
				if choose_mat < 0.8 {
					...
					sphere_material =
						world.add_material(Material::Lambertian { albedo });
					...
					sphere_material =
						world.add_material(Material::Metal { albedo, fuzz });
					world.add_sphere(Sphere::new(center, 0.2, sphere_material));
				} else {
					// glass
					sphere_material =
						world.add_material(Material::Dielectric {
	...
	let material1 = world.add_material(Material::Dielectric {
		refraction_index: 1.5,
	});
